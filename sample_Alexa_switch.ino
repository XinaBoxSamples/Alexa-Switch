...
/* Firmware version */
char*   VERSION_REV = "1.0.0";

/* Smart Config Data */
struct userData {
  char  SSID[32] = "$!$SSID!----------------------!";
  char  PSK[42] = "$!$Password!----------------------------!";
  char  SwitchName[32] = "$!$Switch name!---------------!";
  char  OnWord[32] = "$!$Word for On!---------------!";
  char  OffWord[32] = "$!$Word for Off!--------------!";
};
...
String friendlyName;        // CHANGE: Alexa device name
...
//const char* ssid = "Your SSID";  // CHANGE: Wifi name
...
void setup() {
  ...
  if (ZeroTouch.begin(user.SSID, user.PSK))
  {
    #ifdef DEBUG
  ...
  if(ZeroTouch.checkConfig(user.SwitchName)){
    friendlyName = user.SwitchName;
  } else {
    friendlyName = "FAN";
  }
